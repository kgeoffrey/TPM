...
class tpm_test:
    ...
    def __init__(self):
        self.user = None
        self.url = 'http://127.0.0.1:5000' #'https://tpmserver.herokuapp.com' #"http://127.0.0.1:5000"
        self.secret = None
    ...
    def check_sync(self):
        #print('chaos output is', self.tpm.chaosmap())
        obj = {
            'output' : self.tpm.chaosmap(),
            'user' : self.user
        }

        response = self.checkResponse(requests.post, self.url + '/receive_sync/' + self.secret, obj, 200)
        print('3.5 sync sent')
        response = self.checkResponse(requests.get, self.url + '/check_sync/' + self.secret, None, 200)

        json_ = response.json()
        print('4 sync received', json_)
        if json_['output']['A'] == json_['output']['B']:
...
if __name__ == "__main__":
    ...
    #print("paired!")
    #print('chaotic output', instance.tpm.chaosmap())
